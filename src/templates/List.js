...
const List = ({ items, onRemoveItem, onClick, onEditItem }) => {
  return (
      <ul className="space-y-2 w-full max-w-md">
        {items.map((item) => (
          <li key={item.id} className="py-2 px-4 rounded-lg bg-slate-400 hover:bg-slate-600 focus:ring-2 focus:ring-slate-500 transition-all duration-300 flex justify-between items-center" onClick={() => onClick(item)}>
