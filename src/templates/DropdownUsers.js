...
const DropdownUsers = () => {
  ...
  useEffect(() => {
    if (dropdownRef.current) {
      ...
      const longestTextWidth = users.reduce((maxWidth, user) => {
        const textWidth = context.measureText(user.username).width; // Změněno na 'user.username'
  ...
  const handleAddUser = () => {
    ...
    if (newUser.trim() && !users.some(user => user.username === newUser)) { // Opraveno pro kontrolu na 'username'
      setUsers([...users, { username: newUser }]); // Přidáme objekt s 'username'
      setNewUser('');
    } else if (users.some(user => user.username === newUser)) {
  ...
  const handleRemoveUser = (userToRemove) => {
    setUsers(users.filter((user) => user.username !== userToRemove.username)); // Opraveno pro porovnání 'username'
  };
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      {isOpen && (
        ...
        >
          <ul
            className="h-48 py-4 overflow-y-auto text-gray-700 dark:text-gray-200"
          ...
          >
            {users.map((user, index) => (
              ...
              >
                <span>{user.username}</span> {/* Zobrazíme pouze 'username' */}
          ...
          <div className="p-3 border-t border-gray-200 bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
            ...
            <div className="flex flex-col items-center mt-3 space-y-2">
              <Link
                to="/Account"
                className="bg-blue-600 text-white px-3 py-1 rounded-lg hover:bg-blue-700 w-full text-center"
              >
                Account
              </Link>
              <button
                onClick={handleLogout}
                className="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 w-full text-center mt-2"
              >
                Logout
              </button>
            </div>
